...
class IrodsRestStrategy {
    ...
    downloadFileContent(fileUri, remoteRes, callback) {
        ...
        let getOptions = {
            uri: _.assign(_.clone(this.restURL), {path: this.restURL.path + '/fileContents' + fileUri}),
            /*
            uri: url.format({
                hostname: this.restURL.hostname,
                port: this.restURL.port,
        ...
        request(getOptions, callback).pipe(remoteRes).on('response', function(res) {
            let pathFrags = fileUri.split("/");
